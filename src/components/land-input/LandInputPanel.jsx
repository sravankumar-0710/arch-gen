...
// Purpose: Complete land input interface with canvas, controls, and save button
...
export default function LandInputPanel({ onSave, isLoading }) {
  const { polygonPoints, roadSide, northAngle, unit, isClosed } = useLandStore()
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Title */}
      <div>
        <h2 className="font-serif text-2xl font-normal text-[#f0ede8] mb-2">
          Define Your Plot
        </h2>
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="bg-[#0f0f12] border border-white/[0.06] rounded-xl p-4 flex flex-col gap-4">
          ...
          {(!isClosed || roadSide === null) && (
            <p className="text-xs text-[#5a5855] m-0 text-center">
              {!isClosed
                ? 'Close your polygon to continue'
                : 'Select a road side to continue'}
            </p>
          )}
        </div>
      </div>
